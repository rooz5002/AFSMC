...
class AFSMCApp(tk.Tk):
    ...
    def _save_params(self):
        try:
            # Collect params
            params = {
                "controller": self.ctrl_frame.get_params().__dict__,
                "robot": self.robot_frame.get_params()[0].__dict__,
                "motor": self.robot_frame.get_params()[1].__dict__,
                "case": self.case_frame.get_params().__dict__,
            }

            filename = filedialog.asksaveasfilename(
                initialfile="afsmc_params.json",      # FIXED: was initialname
    ...
    def _load_params(self):
        """Load params from JSON and update all frames."""
        ...
        try:
            ...
            self.ctrl_frame.set_params(ctrl_data)   # this one already exists

            # === 2. Robot & Motor frame (THE FIX) ===
            robot_data = data.get("robot", {})
            motor_data = data.get("motor", {})

            # These two methods definitely exist in your RobotMotorFrame
            self.robot_frame.set_robot_params(robot_data)   # <-- NEW
            self.robot_frame.set_motor_params(motor_data)   # <-- NEW

            # === 3. Case frame ===
            #case_data = data.get("case", {})
            #self.case_frame.set_params(case_data)
            self.case_frame.set_params(data.get("case", {}))
